...
CONFIG = {
    ...
    'mouse': {
        'enabled': True,
        'interval': 3.0,
        'switch_time': 400.0,  # 新增：切换到escape_options的时间(秒)
        'continue_options': [
...
class AutoGameController:
    ...
    def mouse_worker(self):
        ...
        while self.running and self.config['mouse']['enabled']:
            ...
            try:
                ...
                current_options = []
                if (elapsed >= self.switch_time and self.use_escape):
                    current_options = self.config['mouse']['escape_options']
                else:
                    current_options = self.config['mouse']['continue_options']
    ...
    def on_press(self, key):
        ...
        if key == self.config['global']['exit_key']:
            ...
            if self.paused:
                ...
                # self.use_escape = False  # 重置为使用continue_options
    ...
    def start(self):
        ...
        # self.use_escape = False  # 初始使用continue_options
